...
//void turnOnLED(I2C_HandleTypeDef *hi2c);
//void turnOffLED(I2C_HandleTypeDef *hi2c);

/*
writeRegisterByte(I2C_HandleTypeDef *hi2c, uint8_t address, uint8_t value)
{
  buffor[0] = value;
...
void turnOnLED(I2C_HandleTypeDef *hi2c)
{
  ...
  HAL_I2C_Mem_Write(hi2c, SPECTR_ADDR, CFG0_ADDR, 1, buffor, 1, AS7341_CMD_DURATION_USEC);

  buffor[0] = TURN_ON_LED;
  HAL_I2C_Mem_Write(hi2c, SPECTR_ADDR, CONFIG_ADDR, 1, buffor, 1, AS7341_CMD_DURATION_USEC);

  buffor[0] = LED_VAL;
  HAL_I2C_Mem_Write(hi2c, SPECTR_ADDR, LED_ADDR, 1, buffor, 1, AS7341_CMD_DURATION_USEC);
}
void turnOffLED(I2C_HandleTypeDef *hi2c)
{
  uint8_t buffor[1];
